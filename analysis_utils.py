...
#   df.index = np.arange(len(df))
    df = df.groupby(['epoch', 'batch']).mean(numeric_only=True).reset_index()
...
def load_log(run_dir):
    ...
    for split in ['train', 'val', 'test']:
        log_path = os.path.join(run_dir, 'logs', f'{split}.csv')
        if os.path.exists(log_path):
            df = sanitize_df(
                pd.read_csv(log_path))
            dfs.append(df)
        else:
            print(f'Could not find {log_path}, try again')
...
def plot_train_val_losses(ax, train_df, val_df, y_cols, color, title, x_column=None, x_cumsum=False,
    ...
    mask = np.invert(np.isnan(df[val_col]))
    mask = df[val_col] > 0
    mask = np.invert(np.isnan(df[train_col]))
    mask = df[train_col] > 0
    batches_per_epoch = df.shape[0] / np.max([df["epoch"].values])
    print("BPE", batches_per_epoch)
    print("BE", df.shape[0] / batches_per_epoch)
    if x_column is None:
        x = np.arange(df.shape[0]) / batches_per_epoch
        xlabel = 'batch'
    else:
        ...
        if x_cumsum:
            x = np.cumsum(x)
        xlabel = x_column
    if plot_val: ax.plot(x[mask], df[val_col][mask], color=color, label=val_col)
    if plot_train: ax.plot(x[mask], df[train_col][mask], linestyle='--', color=color, label=train_col, alpha=0.5)
    ...
    if len(y_cols) > 1:
        ...
        if plot_val: ax2.plot(x[mask], df[val_col][mask], color=color, label=val_col)
        if plot_train: ax2.plot(x[mask], df[train_col][mask], linestyle='--', color=color, label=train_col, alpha=0.5)
