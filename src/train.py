...
def train(model, criterion, dataset,
    ...
    best_val_acc = 0
    previous_adv_probs = None
    for epoch in range(epoch_offset, epoch_offset+args.n_epochs):
        logger.write('\nEpoch [%d]:\n' % epoch)

        if args.train_epoch_size:
            num_subepochs = math.ceil(len(dataset["train_data"]) / args.train_epoch_size)
        else:
            num_subepochs = 1
        for subepoch in range(num_subepochs):
            logger.write('Sub-Epoch {}/{}\n'.format(subepoch + 1, num_subepochs))
            ...
            if args.resample:
                ...
                val_loss_computer = LossComputer(
                    ...
                    step_size=args.robust_step_size,
                    alpha=args.alpha)
                if previous_adv_probs is not None:
                    val_loss_computer.adv_probs = previous_adv_probs
                run_epoch(
                    epoch, model, optimizer,
                    dataset['val_loader_small'],
                    val_loss_computer,
                    logger, val_csv_logger, args,
                    is_training=False)

                adv_probs = val_loss_computer.manual_update(val_loss_computer.avg_group_loss)
                previous_adv_probs = adv_probs

                if args.partitioner:
                    dataset["train_loader"].sampler.weights = dataset["train_data"].get_partitioner(adv_probs.cpu()).cuda()
                else:
                    group_weights = len(dataset["train_data"]) / dataset["train_data"]._group_counts
                    group_weights = group_weights.cuda() * adv_probs
                    dataset["train_loader"].sampler.weights = group_weights[dataset["train_data"]._group_array]
